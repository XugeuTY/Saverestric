...
from modules import (
    ...
    vision, nidhi, cpfree, allen, tarun, iqfree, webs, ifas, pathfree, forum,
    allenv2, token, abhinavfree, insight, vajiram, qualityfree, jrffree
)
import key, msg, db
...
async def start_app(bot, m):
    if not await join_channel_if_needed(bot, m):
        return
    user_id = int(m.chat.id)
    await bot.send_photo(chat_id=user_id,photo=await key.send_random_photo(),caption=msg.APP,reply_markup=key.home())
...
async def callback_handler(bot, callback_query):
    ...
    if data == "home":
        await call_msg.edit_reply_markup(key.home())
    elif data == "close":
        await call_msg.delete()
    elif data.startswith("ack_page"):
        page = int(data.split("_")[-1]) - 1
        await call_msg.edit_reply_markup(key.gen_app_kb(page))
        ...
        await a("You choose ClassPlus Token Generator", show_alert=True)
        await token.cp_token(bot, call_msg)
    elif data == "insight":
        await a("You choose Insight IAS ", show_alert=True)
        await insight.insight_ias(bot, call_msg)
    elif data == "jrffree":
        ...
        await a("You choose Forum IAS ", show_alert=True)
        await forum.forum_Ias(bot, call_msg)
    elif data == "vision":
        ...
        await appx_master.handle_appx_logic(bot, call_msg)
    elif data == "pw":#D
        await a("You choose Physics Wallah ", show_alert=True)
        await pw.handle_pw_logic(bot, call_msg)
    elif data == "webs":
        await a("You choose WebSankul Extractor ", show_alert=True)
        await webs.web_sankul_logic(bot, call_msg)
    elif data == "cp":#D
        ...
        await a("You choose Tarun Grover Sir", show_alert=True)
        await tarun.handle_tarun_logic(bot, call_msg)
